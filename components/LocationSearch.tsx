    ...
    <div ref={containerRef} className="relative">
      <div className="relative">
        ...
        <input
          ...
          required={required}
          autoComplete="off"
          className={`w-full rounded-xl pl-9 pr-8 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-[#FFC531] focus:border-transparent transition-colors ${
            dark
              ? "border border-white/20 bg-white/10 text-white placeholder-white/40"
              : "border border-gray-200 focus:border-[#FFC531] focus:ring-[#FFC531]/20"
          }`}
        />
        {loading && (
          <span className="absolute right-3 top-1/2 -translate-y-1/2">
            <svg className="w-4 h-4 animate-spin text-[#FFC531]" fill="none" viewBox="0 0 24 24">
          ...
          {suggestions.map((s, i) => (
            <li key={s.place_id || i}>
              <button
                ...
                className="w-full text-left px-4 py-3 hover:bg-[#FFC531]/10 transition-colors border-b border-gray-100 last:border-0"
              >
