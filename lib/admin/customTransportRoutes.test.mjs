...
test("Daftar Harga wires custom route create, edit, save, and delete", async () => {
  const source = await readFile(
    new URL(
      "../../components/admin/views/PriceListView.tsx",
      import.meta.url
    ),
    "utf8"
  );

  assert.match(source, /<AddTransportRouteForm/);
  assert.match(source, /\.from\("custom_transport_routes"\)\.insert/);
  assert.match(source, /\.from\("custom_transport_routes"\)\.upsert/);
  assert.match(source, /\.from\("custom_transport_routes"\)\s*\.delete/);
  assert.match(source, /customRoute\.group_name/);
  assert.match(source, /deleteCustomRoute/);
});
